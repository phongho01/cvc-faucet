...
const faucet = async (author, address, redis) => {
  ...
  if (times.lte(10)) {
    const text = `Remaining balance of account ${process.env.ACCOUNT_ADDRESS} is just enough to faucet ${Math.floor(times.toNumber())} times (${Math.floor(remainingBalance.toNumber())} ${CURRENCY_SYMBOL}). Please deposit.`;
    sendMessage(text);
  }
